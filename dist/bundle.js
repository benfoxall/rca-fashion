...
var Drop = function () {
  function Drop(element) {
    ...
    var close = function close() {
      ...
      _this.root.parentNode.style.zIndex = 2000;
      state = CLOSED;
    };
    var open = function open() {
      _this.options.style.display = 'block';
      _this.root.parentNode.style.zIndex = 2100;
      state = OPEN;
    };
    ...
    document.body.addEventListener('touchstart', function (e) {
      if (!_this.root.contains(e.target)) close();
    }, false);
  }

  _createClass(Drop, [{
    key: 'match_size',
    value: function match_size() {
      var current_display = this.options.style.display;

      this.options.style.display = 'block';

      this.current.style.width = this.options.style.width = '';

      var width = Math.max(parseFloat(getComputedStyle(this.current).width), parseFloat(getComputedStyle(this.options).width));
...
var drops = Array.from(qsa('.drop')).map(function (drop) {
...
Array.from(document.querySelectorAll('#asset-list li')).forEach(function (img) {
